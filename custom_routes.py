...
async def send_json_override(self, event, data, sid=None):
    ...
    if event == "executed" and "node" in data and "output" in data:
        await update_run_with_output(
            prompt_id, data.get("output"), node_id=data.get("node")
        )
...
def update_run(prompt_id, status: Status):
    ...
    if (
        "status" not in prompt_metadata[prompt_id]
        or prompt_metadata[prompt_id]["status"] != status
    ):

        # when the status is already failed, we don't want to update it to success
        if (
            "status" in prompt_metadata[prompt_id]
            and prompt_metadata[prompt_id]["status"] == Status.FAILED
        ):
            return
...
async def upload_file(
    prompt_id, filename, subfolder=None, content_type="image/png", type="output"
):
    """
...
def have_pending_upload(prompt_id):
    if (
        ...
        and "uploading_nodes" in prompt_metadata[prompt_id]
        and len(prompt_metadata[prompt_id]["uploading_nodes"]) > 0
    ):
        print(
            "have pending upload ", len(prompt_metadata[prompt_id]["uploading_nodes"])
        )
        return True
...
async def update_file_status(
    prompt_id, data, uploading, have_error=False, node_id=None
):
    ...
    try:
        ...
        for file in files:
            await upload_file(
                ...
                type=file.get("type"),
                content_type=file.get("content_type", "image/png"),
            )
        ...
        for gif in gifs:
            await upload_file(
                ...
                type=gif.get("type"),
                content_type=gif.get("format", "image/gif"),
            )
...
async def update_run_with_output(prompt_id, data, node_id=None):
    if prompt_id in prompt_metadata:
        ...
        body = {"run_id": prompt_id, "output_data": data}
        ...
        try:
            ...
            asyncio.create_task(
                upload_in_background(
                    prompt_id, data, node_id=node_id, have_upload=have_upload
                )
            )
